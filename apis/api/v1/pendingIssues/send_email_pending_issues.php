    ...
    for($i=0; $i < count($arr_users_data['data']); $i++) {
        ...
        for ($k=0; $k < count($arr_list); $k++) {
            if($k==0 && $k+1 == count($arr_list)){
                ...
                $pendingIssuesList = $pendingIssuesList .= '</ul>'; 
            } else {
                $pendingIssuesList = $pendingIssuesList .= $arr_list[$k];
            }
        }
        $body = '<b>Olá, '.$arr_users_data['data'][$i]['name'].'</b><br>Segue abaixo as suas pendências ainda não concluídas: <br>'.$pendingIssuesList;
